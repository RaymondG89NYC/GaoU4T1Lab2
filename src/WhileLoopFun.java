public class WhileLoopFun {
    ...
    public void printDigits(int number) {
        String numberWord = "" + number;
        int length = numberWord.length();
        while (length > 0){
            System.out.println(numberWord.substring(length-1, length));
            length--;
        }
    }
    ...
    public int countLetter(String word, String letter) {
        ...
        while (x < word.length()) {
            if (word.substring(x, x + 1).contains(letter)) {
                count++;
            }
            x++;
        }
    ...
    public int maxDoubles(int number, int threshold) {
        int count = 0;
        while ((number * Math.pow(2, (count+1))) <= threshold){
            count++;
        }
        return count;
    }
